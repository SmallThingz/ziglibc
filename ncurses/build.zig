...
const NcursesPrepStep = struct {
    ...
    fn generateNcursesDefH(self: NcursesPrepStep) !void {
        ...
        if (try filecheck.leftFileIsNewer(io, self.defs_h_dst, self.defs_h_src))
            return;
        var arena = std.heap.ArenaAllocator.init(std.heap.page_allocator);
        defer arena.deinit();
        const content = cwd.readFileAlloc(io, self.defs_h_src, arena.allocator(), .unlimited) catch |err| {
        ...
        try cwd.rename(tmp_filename, cwd, self.defs_h_dst, io);
    }
};
...
pub fn add(
    ...
    addIncludePathCompat(exe, lazyPath(b, b.pathJoin(&.{ repo_path, "ncurses" })));
    ...
    //addIncludePathCompat(exe, .{.path = "inc/gnu"});
