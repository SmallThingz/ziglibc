...
int main(void) {
  ...
  { FILE *f=fopen("posix-ext.tmp","w"); TEST_MARK_IF_ENV("ZIGLIBC_TEST_MARKERS", "posix_file:access:fopen"); expect(f!=NULL); TEST_MARK_IF_ENV("ZIGLIBC_TEST_MARKERS", "posix_file:access:fileno"); expect(fileno(f)>=0); TEST_MARK_IF_ENV("ZIGLIBC_TEST_MARKERS", "posix_file:access:fclose"); expect(0==fclose(f)); TEST_MARK_IF_ENV("ZIGLIBC_TEST_MARKERS", "posix_file:access:present"); expect(0==access("posix-ext.tmp",R_OK)); TEST_MARK_IF_ENV("ZIGLIBC_TEST_MARKERS", "posix_file:access:missing"); expect(-1==access("missing-posix-ext.tmp",R_OK)); TEST_MARK_IF_ENV("ZIGLIBC_TEST_MARKERS", "posix_file:access:unlink"); expect(0==unlink("posix-ext.tmp")); expect(-1==unlink("posix-ext.tmp")); }
  ...
  { struct iovec outv[2], inv[2]; char a[4]={0}, b[4]={0}; ssize_t wr; fd=open("posix-writev.tmp",O_CREAT|O_TRUNC|O_RDWR,0600); if(fd<0){fprintf(stderr,"posix_file:writev-open-create errno=%d\n",errno); return 1;} outv[0].iov_base=(void*)"ab"; outv[0].iov_len=2; outv[1].iov_base=(void*)"cd"; outv[1].iov_len=2; wr=writev(fd,outv,2); if(wr!=4){fprintf(stderr,"posix_file:writev-create rc=%lld errno=%d\n",(long long)wr,errno); return 1;} expect(0==close(fd)); fd=open("posix-writev.tmp",O_RDONLY); if(fd<0){fprintf(stderr,"posix_file:writev-open-read errno=%d\n",errno); return 1;} inv[0].iov_base=a; inv[0].iov_len=2; inv[1].iov_base=b; inv[1].iov_len=2; expect(4==readv(fd,inv,2)); expect(0==memcmp(a,"ab",2)); expect(0==memcmp(b,"cd",2)); expect(0==close(fd)); expect(0==unlink("posix-writev.tmp")); }
